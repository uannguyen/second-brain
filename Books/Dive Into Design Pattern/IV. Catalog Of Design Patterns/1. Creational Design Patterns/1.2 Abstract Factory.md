...
- 1. Vạch ra matrix của các loại products riêng biệt so với các variants của các products này
- 2. Declare abstract product interfaces cho tất cả product types
- 3. Declare the abstract factory interface với một tập hợp creation methods cho tất cả abstract products
- 4. Implement một tập hợp concrete factory classes, một cho mỗi product variant
- 5. Tạo factory initialization code ở đâu đó trong app, nó sẽ khởi tạo một trong các concrete factory classes, tùy thuộc vào configuration or env hiện tại
	- Truyền factory object này cho tất cả các classes construct products
- 6. Scan code & tìm tất cả các cuộc gọi trực tiếp đến các product constructors. Thay thế chúng bằng các cuộc gọi đến creation method trên factory object
...
- Ưu điểm:
	...
	- Tránh khớp nối chặt chẽ giữa concrete products & client code
		...
		- `Open/Closed Principle`: Có thể proce
