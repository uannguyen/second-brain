...
- Phải define được các bước xây dựng chung chung cho các biểu diễn sản phẩm có sẵn

- Declares các steps này trong base builder interface. Các class builder sẽ implement interface này.

- Tạo 1 class builder cụ thể(concrete builder) cho mỗi loại sản phẩm(product representaions) và implement các construction steps
