...
- Observer pattern suggests rằng bạn thêm cơ chế đăng ký(subscription mechanism) cho the publisher class để các individual objects có thể subscribe or unsubcribe khỏi stream of events đến từ publisher đó
	- Trong thực tế, cơ chế này được triển khai:
		- 1) Một array field để lưu trữ một list of references to subcriber objects
		- 2) Thêm một vài public methods cho phép adding subcribers & removing chúng khỏi list
...
- Đó là lí do tại sao all subcribers nên implement chung một interface & publisher communicates with subscribers thông qua interface
...
- Nếu app có nhiều loại publishers khác nhau và muốn tất cả các subcribers đều có thể follow nó thì các publishers này nên implement cùng một interface
	- Interface này chỉ cần mô tả một vài subscription method
	- Interface cho phép subcribers quan sát publisher's states mà không coupling với concrete class 
